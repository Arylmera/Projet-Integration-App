        ...
        firebase.auth().onAuthStateChanged((user) => {
            let prenom = user.displayName.split(' ')[1];
                user
                    ...
                    .then(() => {
                        this.setState({infoText: 'Un nouveau lien a été envoyé !', prenom: prenom})
                    })
                    .catch((error) => {
                        console.log(error);
                    });
        ...
        return (
                <View style={styles.main_container}>
                    ...
                    <Text style={styles.textExplication}>
                        Un email vous a été envoyé pour vérifier votre adresse email, veuillez valider l'adresse et relancer l'application.
                    </Text>
                    </View>
                    <View style={styles.text_container}>
                    <Text>
                        Si vous désirez un nouveau lien de confirmation appuyer sur le bouton
                    </Text>
                    <TouchableOpacity
                        style={styles.button}
                        onPress={() =>
                            this._reenvoyerConfirmation(
                            )
                        }>
